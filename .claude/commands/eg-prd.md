...
- `multiSelect`: `true`
- `options`:
  1. **Save as `notes/prds/<slug>-<YYYY-MM-DD>.md`** — "Durable artifact. Recommended for anything Standard or Comprehensive."
...
- Patterns to mirror: <e.g. "uses SQLAlchemy sessions with explicit close(); ADP wall enforced by test_leakage.py">
- Constraints from CLAUDE.md or recent decisions: <ADP wall, data leakage rules, snapshot_id requirement, config coupling, etc.>
...
- **Save as `notes/prds/<slug>-<YYYY-MM-DD>.md`**: write the PRD to disk. Derive the slug from the PRD title (kebab-case, lowercase) and date from `date +%Y-%m-%d`. If the directory doesn't exist, create it. Confirm the path back to the user.
...
**STOP.** Do NOT commit; auto mode does not override the project's commit policy. If the PRD was saved to disk, it's a new file the user will commit themselves when ready. Follow `git log` convention: terse lowercase.
