...
URI, port 5432 — direct connection is IPv6-only and fails on IPv4 Macs),
`RESEARCH_DATABASE_URL=postgresql://localhost:5432/ff_research` (local Postgres),
and `CFBD_API_KEY=...`. DB engines are lazy: imports never need a URL.

**Two DB roles** (`src/db/session.py`): `session_scope()` defaults to **research**
(local PG, full history — pipeline/training/benchmark); `serving_session_scope()`
and the API's `SessionLocal` hit **serving** (Supabase, tiny subset). Keep Supabase
under the free-tier 500 MB cap: the pipeline writes research, then
`scripts/publish_serving.py` copies the serving subset (players, adp, projections,
weekly_projections, current-season weekly_features, latest snapshot) to Supabase.
`RESEARCH_DATABASE_URL` unset → both roles are the same DB (single-DB/test mode).
Serving DB has **no** raw box scores / labels / full weekly_features — never point
the pipeline or a benchmark at serving.
