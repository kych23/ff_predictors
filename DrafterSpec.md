...
src/
  ...
  features/      prior_production.py  role_change.py  rookies.py
                 team_context.py  assemble.py  leakage_guard.py  __init__.py
...
| `src/features/team_context.py` | NEW (post-v1) | **Market team scoring context**: the offense a player sits in, from the **Week-1 betting line** (nflverse `total_line`/`spread_line`) — `team_ctx_implied_pts = total_line/2 ± spread_line/2`, plus the game total and the team's spread. Week-1 lines post pre-kickoff, so it's a **preseason-safe artifact** (like the Week-1 depth chart), attached by season-Y team. A deliberate *game-line* market signal in the features — distinct from the ADP wall (which bans the **draft** market); uses `total_line`/`spread_line` only, never `total` (the final score). Gate-validated: lifts Tier-2, flat on Tier-1. |
...
**`team_context.py` — market team scoring context** (post-v1; preseason-safe, §4.0):

- `team_ctx_implied_pts` (market-implied points for the player's team in its season-`Y` **Week-1** game = `total_line/2 ± spread_line/2`; `spread_line` is home-favored-positive), `team_ctx_game_total` (the Week-1 over/under — scoring environment / pace), `team_ctx_spread` (points favored `+` / underdog `−` for the team). Source: nflverse schedules `total_line`/`spread_line` (**never `total`**, the final score). Week-1 lines post before kickoff, so this is a current-season **preseason artifact** (NOT routed through the prior-season leakage filter), attached by season-`Y` team. A deliberate game-line **market** signal — separate from the ADP/draft-market wall — that prices offseason roster/coaching change lagged production can't see.
...
| `scripts/draft.py` | NEW | The live draft-day CLI: reads draft state via `draft_state_source.py` (Sleeper live poll or manual entry, §4.8.1), and at each of my picks prints the **top-10 recommendation board with FIT scores + a separation read** (§4.8.1) so I decide rather than blindly taking #1. Manual mode adds `undo`, crash-safe `--resume`, a `roster` view, positional run-detection, and an ADP column. |
| `scripts/mock_draft.py` | NEW | Replays a COMPLETED season as a scored mock draft (recommender vs ADP bots, scored against actual FPPG labels); reuses the Tier-2 `draft_sim` engine. Single-slot detail or `--all-slots` summary. |
| `scripts/run_pipeline.sh` | NEW | One-shot pipeline driver from an empty DB: seed → labels → features → train → benchmark. |
...
**Top-10 recommendation display (decision support, not auto-pick).** At each of my picks the tool surfaces the **top 10** candidates by VONA, each with a **FIT** value (the player's VONA score — marginal lineup value minus the wait term) and **Δ#1** (gap to the top pick). The point is to keep the human in the loop: the *spread* of FIT across the 10 reveals whether one pick clearly dominates or the choices are interchangeable. A one-line **read** auto-classifies it — *CLEAR #1* when the leader beats #2 by ≥ 1.0, *TOSS-UP* when the top 10 fall within 0.5 (pick on preference), else a *slight edge*. The recommender ranks; the drafter chooses.
