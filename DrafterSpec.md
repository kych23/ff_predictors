...
| `scripts/draft.py` | NEW | The live draft-day CLI: reads draft state via `draft_state_source.py` (Sleeper live poll or manual entry, §4.8.1), and at each of my picks prints the **top-5 recommendation board with FIT scores + a separation read** (§4.8.1) so I decide rather than blindly taking #1. |
...
**Top-5 recommendation display (decision support, not auto-pick).** At each of my picks the tool surfaces the **top 5** candidates by VONA, each with a **FIT** value (the player's VONA score — marginal lineup value minus the wait term) and **Δ#1** (gap to the top pick). The point is to keep the human in the loop: the *spread* of FIT across the 5 reveals whether one pick clearly dominates or the choices are interchangeable. A one-line **read** auto-classifies it — *CLEAR #1* when the leader beats #2 by ≥ 1.0, *TOSS-UP* when the top 5 fall within 0.5 (pick on preference), else a *slight edge*. The recommender ranks; the drafter chooses.
