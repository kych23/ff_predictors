...
PORTED from src/features/prior_production.py (DraftEngineDesign.md §9.2).
...
def _season_aggregates(stats: pd.DataFrame, pfr_to_gsis: dict[str, str] | None,
    ...
    if stats.empty:
        return pd.DataFrame()
    df = stats.copy()
    df["week_points"] = score_offense(df, offense)
    for c in _SUM_COLS:
...
def build_prior_production(
    ...
    *,
    ...
    if offense is None:
        raise ValueError(
            "build_prior_production needs the league's offensive scoring "
            "coefficients. v1 reached for global config here; the model layer "
            "may not, so the caller passes them (§9.0)."
        )
