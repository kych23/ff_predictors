...
import { TIER_DETAIL, TIER_LABEL, tierIsFallback } from "../tiers";
      ...
      <p className="tnum mt-1 text-lg">
        ...
        <span
          className={`ml-3 rounded border px-2 py-0.5 text-xs ${
            tierIsFallback(rec.tier)
              ? "border-warn/50 bg-warn/10 text-warn"
              : "border-line text-muted"
          }`}
          title={TIER_DETAIL[rec.tier] ?? ""}
        >
          {TIER_LABEL[rec.tier] ?? `tier ${rec.tier}`}
        </span>
      </p>
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden">
      ...
      <div className="flex min-h-[8rem] flex-1 flex-col overflow-hidden lg:flex-row">
        ...
        {/* The right rail is the context column: why the engine picked this,
            and the reads it cannot see. */}
        <aside className="flex w-72 shrink-0 flex-col overflow-y-auto border-l border-line">
          ...
          <div className="px-4 pb-4">
            <NotesPanel sessionId={state.session_id} />
          </div>
        </aside>
      </div>
